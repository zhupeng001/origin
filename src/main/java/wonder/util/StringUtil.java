...
public class StringUtil {
	...
	public static String null2StrNoTrim(String value) {
	...
	public static Map<String, String> parseUrlParams(String url) {
		...
		String paramsStr = url.contains("?") ? url
	...
	public static String clearAsciiControl(String s) {
		return s.replaceAll("[\\x00-\\x1f]", s);
	}
	...
	public static String changeSpace(String html, boolean fromClt, boolean toClt) {
		if (html == null)
			return html;
		if (toClt) {
			if (!fromClt) {
			...
			if (fromClt) {
				html = html.replaceAll("\\s{1,2}", "　");// 注意替换成全角空格
			} else {
				html = html.replaceAll("\\s{1,4}", "　");// 注意替换成全角空格
			}
		}
		return html;
	}
	...
	public static String removeUnicodeChar(String str) {
		if (str == null)
			return str;
		// u2028:行分隔符 ; u2029: 段落分隔符; ufeff: bom，往往隐藏在utf-8文件开头
		str = str.replaceAll("[\\u2028|\\u2029|\\ufeff]", "");
		return str;
	}

	public static double nullToDouble(Object value) {
		return value == null || "null".equals(value.toString()) || "".equals(value.toString()) ? 0.0 : stringToDouble(value.toString().trim());
	}
	 ...
	 * @return
	 */
	public static String getTextx(String str) {
		if (str == null)
			return ("");
		if (str.equals(""))
			return ("");
	...
	public static void main(String[] args) {
		HashMap<String, String> characMap = new HashMap<String, String>();
		characMap.put("'", "\\\'");
		String name = "'诗人'贺炜安慰了西班牙";
		System.out.println(name);
		name = StringUtil.getText(name);
		System.out.println(name);
		name = StringUtil.escapeCharacter(name, characMap);
		System.out.println(name);
